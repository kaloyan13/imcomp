...
void findAffineRegion(DARY *image, vector<CornerDescriptor*> &cor)
{
	vector<CornerDescriptor *>::iterator it;
	for (it = cor.begin(); it != cor.end(); ) {
		if ((*it)->getCornerScale() < 25) {
			int l = findAffineRegion(image, (*it));
			if (l == 0) {
				it = cor.erase(it);
			}
			else {
...
void findScaleRegion(DARY *image, vector<CornerDescriptor*> &cor) {
	vector<CornerDescriptor *>::iterator it;
	for (it = cor.begin(); it != cor.end(); ) {
		int l = (int)searchDerScale(image, (*it));
		if (l == 0) {
			it = cor.erase(it);
		}
		else {
