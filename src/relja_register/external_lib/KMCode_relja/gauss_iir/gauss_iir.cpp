...
float smooth(int x, int y, DARY* image_in, float scale) {
	DARY* image = new DARY(2 * asize + 1, 2 * asize + 1);
	DARY* image_tmp = new DARY(2 * asize + 1, 2 * asize + 1);

	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++) {
			image->fel[i + asize][j + asize] = image_in->fel[y + i][x + j];
		}
	...
	float ret = image->fel[asize][asize];
...
float smooth(int x, int y, DARY* image_in, float scalex, float scaley) {
	...
	*/
	...
	float value = 0;
	//if(x<asize || y<asize || x+asize>=image_in->x() || y+asize>=image_in->y())return(value);
	for (int i = -sizey; i <= (int)sizey; i++)
...
void initGauss(float scale) {
	...
	g_scale = scale;
	asize = (int)rint(GAUSS_CUTOFF*g_scale);
	if (table_exp != NULL) { delete[] table_exp[0]; delete[] table_exp; }
	table_exp = new float*[(2 * asize + 1)];
	table_exp[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp[i] = table_exp[0] + i*(2 * asize + 1);
	...
	for (int i = 1; i <= (size_loc - asize); i++) {
		g_loc[asize - i] += g_loc[asize + i];
	}
	for (int i = 0; i <= asize; i++)
		for (int j = 0; j <= asize; j++) {
			table_exp[i + asize][-j + asize] = table_exp[-i + asize][j + asize] =
				table_exp[i + asize][j + asize] = table_exp[-i + asize][-j + asize] =
				g_loc[i] * g_loc[j];
		}
	/*	for (int i=0;i<=asize;i++)
	...
	*/
	/*for (int i=-asize;i<=asize;i++)
	  for (int j=-asize;j<=asize;j++){
	  table_exp[i+asize][j+asize] =
	  exp(((i*i+j*j)/(h_square_scale)));
	  }*/
	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++)total += table_exp[i + asize][j + asize];
	delete[]g_loc;
	//write(table_exp,"gauss_test.mat");
		//cout <<"total "<< total<< endl;//getchar();
}
...
void initGaussX(float scale) {
	...
	table_exp_x = new float*[(2 * asize + 1)];
	table_exp_x[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_x[i] = table_exp_x[0] + i*(2 * asize + 1);

	float square_scale = x_scale * x_scale;

	/*	for (int i=-asize;i<=asize;i++)
		for (int j=-asize;j<=asize;j++)
		table_exp_x[j+asize][i+asize] =
		(-i/(square_scale))*table_exp[j+asize][i+asize];*/
	for (int i = 0; i <= asize; i++)
		for (int j = 0; j <= asize; j++) {
			table_exp_x[-j + asize][i + asize] =
				table_exp_x[j + asize][i + asize] =
			...
			table_exp_x[-j + asize][-i + asize] =
				table_exp_x[j + asize][-i + asize] =
				-1 * table_exp_x[j + asize][i + asize];
		}
	sum_x = 0;
	for (int i = 0; i <= asize; i++)
		sum_x += table_exp_x[asize][i];
...
void initGaussXX(float scale) {
	...
	table_exp_xx = new float*[(2 * asize + 1)];
	table_exp_xx[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xx[i] = table_exp_xx[0] + i*(2 * asize + 1);
	...
	/*	for (int i=-asize;i<=asize;i++)
		for (int j=-asize;j<=asize;j++)table_exp_xx[j+asize][i+asize] =
		(mo_square_scale+(i*i/(square_square_scale)))*
		table_exp[j+asize][i+asize];
	*/
	for (int i = 0; i <= asize; i++)
		for (int j = 0; j <= asize; j++) {
			table_exp_xx[-j + asize][i + asize] =
				table_exp_xx[-j + asize][-i + asize] =
				table_exp_xx[j + asize][-i + asize] =
				table_exp_xx[j + asize][i + asize] =
				(mo_square_scale + (i*i / (square_square_scale)))*
...
void initGaussXY(float scale) {
	...
	table_exp_xy = new float*[(2 * asize + 1)];
	table_exp_xy[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xy[i] = table_exp_xy[0] + i*(2 * asize + 1);

	float square_scale = xy_scale * xy_scale;
	float square_square_scale = square_scale*square_scale;

	/*	for (int i=-asize;i<=asize;i++)
		for (int j=-asize;j<=asize;j++)table_exp_xy[j+asize][i+asize] =
						 (i*j/(square_square_scale))*table_exp[j+asize][i+asize];
	*/
	for (int i = 0; i <= asize; i++)
		for (int j = 0; j <= asize; j++) {
			table_exp_xy[-j + asize][-i + asize] =
				table_exp_xy[j + asize][i + asize] =
				(i*j / (square_square_scale))*table_exp[j + asize][i + asize];
			table_exp_xy[j + asize][-i + asize] =
				table_exp_xy[-j + asize][i + asize] =
				-1 * table_exp_xy[j + asize][i + asize];
		}
...
void initGaussLap(float scale) {
	...
	table_exp_lap = new float*[(2 * asize + 1)];
	table_exp_lap[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_lap[i] = table_exp_lap[0] + i*(2 * asize + 1);

	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++)table_exp_lap[j + asize][i + asize] =
			(table_exp_xx[i + asize][j + asize] + table_exp_xx[j + asize][i + asize]);
...
void initGaussXXX(float scale) {
	...
	table_exp_xxx = new float*[(2 * asize + 1)];
	table_exp_xxx[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xxx[i] = table_exp_xxx[0] + i*(2 * asize + 1);

	float square_scale = xxx_scale * xxx_scale;
	float square_square_scale = square_scale*square_scale;

	/*	for (int i=-asize;i<=asize;i++)
		for (int j=-asize;j<=asize;j++)table_exp_xxx[j+asize][i+asize] =
		(i/square_square_scale)*(3-((i*i)/(square_scale)))*
		table_exp[j+asize][i+asize];
	*/
	for (int i = 0; i <= asize; i++)
		for (int j = 0; j <= asize; j++) {
			table_exp_xxx[-j + asize][i + asize] =
				table_exp_xxx[j + asize][i + asize] =
				(i / square_square_scale)*(3 - ((i*i) / (square_scale)))*
				table_exp[j + asize][i + asize];
			table_exp_xxx[j + asize][-i + asize] =
				table_exp_xxx[-j + asize][-i + asize] =
				-1 * table_exp_xxx[j + asize][i + asize];
...
void initGaussXXY(float scale) {
	...
	table_exp_xxy = new float*[(2 * asize + 1)];
	table_exp_xxy[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xxy[i] = table_exp_xxy[0] + i*(2 * asize + 1);

	float square_scale = xxy_scale * xxy_scale;
	float square_square_scale = square_scale*square_scale;

	/*	for (int i=-asize;i<=asize;i++)
		for (int j=-asize;j<=asize;j++)table_exp_xxy[j+asize][i+asize] =
		(j/square_square_scale)*(1-((i*i)/(square_scale)))*
		table_exp[j+asize][i+asize];
	*/
	for (int i = 0; i <= asize; i++)
		for (int j = 0; j <= asize; j++) {
			table_exp_xxy[j + asize][-i + asize] =
				table_exp_xxy[j + asize][i + asize] =
				(j / square_square_scale)*(1 - ((i*i) / (square_scale)))*
				table_exp[j + asize][i + asize];
			table_exp_xxy[-j + asize][-i + asize] =
				table_exp_xxy[-j + asize][i + asize] =
				-1 * table_exp_xxy[j + asize][i + asize];
		}
...
void initGaussXXXX(float scale) {
	...
	table_exp_xxxx = new float*[(2 * asize + 1)];
	table_exp_xxxx[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xxxx[i] = table_exp_xxxx[0] + i*(2 * asize + 1);

	float square_scale = xxxx_scale * xxxx_scale;
	float o_square_square_scale = (1 / (square_scale*square_scale));

	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++)table_exp_xxxx[j + asize][i + asize] =
...
void initGaussXXXY(float scale) {
	...
	table_exp_xxxy = new float*[(2 * asize + 1)];
	table_exp_xxxy[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xxxy[i] = table_exp_xxxy[0] + i*(2 * asize + 1);

	float square_scale = xxxy_scale * xxxy_scale;
	float square_square_square_scale = square_scale*square_scale*square_scale;

	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++)table_exp_xxxy[j + asize][i + asize] =
...
void initGaussXXYY(float scale) {
	...
	table_exp_xxyy = new float*[(2 * asize + 1)];
	table_exp_xxyy[0] = new float[(2 * asize + 1)*(2 * asize + 1)];
	for (int i = 1; i < (2 * asize + 1); i++)table_exp_xxyy[i] = table_exp_xxyy[0] + i*(2 * asize + 1);
	...
	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++)table_exp_xxyy[j + asize][i + asize] =
...
float convolution(int x, int y, DARY* image_in, float** gauss) {
	...
	float value = 0;
	//cout << x << " " << y << " " << asize << "  " << image_in->x()<< endl;
	if (x < asize || y < asize || x + asize >= (int)image_in->x() || y + asize >= (int)image_in->y())return(value);
	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++) {
			value += image_in->fel[y + j][x + i] * gauss[j + asize][i + asize];
		}
	return(value);
}
...
float inv_convolution(int x, int y, DARY* image_in, float** gauss) {
	...
	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++) {
			value += image_in->fel[y + j][x + i] * gauss[i + asize][j + asize];
		}
	return(value);
...
void drawGauss(DARY* image_in, int x, int y, float scale) {
	...
	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++) {
			if (max < gauss1[i + asize][j + asize])
				max = gauss1[i + asize][j + asize];
			if (min > gauss1[i + asize][j + asize])
				min = gauss1[i + asize][j + asize];
	...
	if (fabs(min) > max)
		...
		sc = 127.0 / max;
	if (x < asize || y < asize || x + asize >= (int)image_in->x() || y + asize >= (int)image_in->y())return;

	for (int i = -asize; i <= asize; i++)
		for (int j = -asize; j <= asize; j++) {
			image_in->fel[y + i][x + j] = 128 + sc*gauss1[i + asize][j + asize];
			//cout <<image_in->fel[y+i][x+j] << endl;
		}
}
