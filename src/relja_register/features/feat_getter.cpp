...
void
featGetter::getFeats( const char fileName[], uint32_t xl, uint32_t xu, uint32_t yl, uint32_t yu, uint32_t &numFeats, std::vector<ellipse> &regions, float **&descs ) {
    std::string fileName_ ( fileName );
    std::cout << "\n***** Processing file : " << fileName_ << " ******" << std::flush;
    std::cout << "\n(xu,yu)=(" << xu << "," << yu << ") (xl,yl)=(" << xl << "," << yl << ")" << std::flush;
    ...
    tmp_dir = tmp_dir / "tmp";
    std::string tempCropImageFn= (tmp_dir / boost::filesystem::unique_path("rr_crop_%%%%-%%%%-%%%%-%%%%.jpg")).string();
    try {
        Magick::Image im;
        im.read(fileName_);
        std::cout << "\nimage dim = " << im.size().width() << "x" << im.size().height() << std::flush;
