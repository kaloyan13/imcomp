...
(cd $BUILD_BASE_FOLDER;
 cmake DCMAKE_C_COMPILER=/usr/local/bin/gcc-6 -DCMAKE_CXX_COMPILER=/usr/local/bin/g++-6 -DCMAKE_BUILD_TYPE=Release -DRELEASE_PACKAGE_NAME=$RELEASE_PACKAGE_NAME -DRELEASE_PACKAGE_DESCRIPTION=$RELEASE_PACKAGE_DESCRIPTION -DIMCOMP_SERVER_VERSION_PATCH=$IMCOMP_SERVER_VERSION_PATCH -DCMAKE_PREFIX_PATH=$IMCOMP_DEPS_DIR $IMCOMP_SOURCE_PATH
)
