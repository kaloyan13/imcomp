...
(cd $BUILD_BASE_FOLDER;
 cmake -DCMAKE_VERBOSE_MAKEFILE:BOOL=OFF -DCMAKE_BUILD_TYPE=Release -DRELEASE_PACKAGE_NAME=$RELEASE_PACKAGE_NAME -DRELEASE_PACKAGE_DESCRIPTION=$RELEASE_PACKAGE_DESCRIPTION -DIMCOMP_SERVER_VERSION_PATCH=$IMCOMP_SERVER_VERSION_PATCH -DCMAKE_PREFIX_PATH=$IMCOMP_DEPS_DIR $IMCOMP_SOURCE_PATH ;
 make -j 8
)
